<template >
<div class="justify-center flex flex-col bg-white items-center h-screen">
<div class="text-4xl">
    <img class="ease-in w-60" src="https://res.cloudinary.com/dw21mdxqj/image/upload/v1634277410/elementos_de_marca_sr-05_ojwejv.png" alt="">
</div>
<div class="animate-bounce ">
    <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M16.707 10.293a1 1 0 010 1.414l-6 6a1 1 0 01-1.414 0l-6-6a1 1 0 111.414-1.414L9 14.586V3a1 1 0 012 0v11.586l4.293-4.293a1 1 0 011.414 0z" clip-rule="evenodd"></path></svg>
</div>
<router-link to="/precios" class="animate-ping">Hola mundo</router-link>
</div>
</template>

<style>
#app {
font-family: Avenir, Helvetica, Arial, sans-serif;
-webkit-font-smoothing: antialiased;
-moz-osx-font-smoothing: grayscale;
text-align: center;
color: #2c3e50;
}

#nav {
padding: 30px;
}

#nav a {
font-weight: bold;
color: #2c3e50;
}

#nav a.router-link-exact-active {
color: #42b983;
}
</style>
